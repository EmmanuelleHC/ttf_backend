...
import (
	...
	"github.com/Aguztinus/petty-cash-backend/pkg/hash"
...
func (a UserService) GetUserMenuTrees(ID string) (models.MenuTrees, error) {
	if a.GetSuperAdmin().ID == ID {
		menuQR, err := a.menuRepository.Query(&models.MenuQueryParam{
			...
			PaginationParam: dto.PaginationParam{Current: 1, PageSize: 128},
		})
	...
	// Get the parent menu of the authorization menu, determine which parent menus are not in the previous authorization menu, and store them in the parentIDs slice
	...
	// Get the information of the parent menu of these differences and add it to menuResult.Data
	if len(parentIDs) > 0 {
...
func (a UserService) GetByUsername(username string) (*models.User, error) {
	if a.GetSuperAdmin().Username == username {
		user := a.GetSuperAdmin()
		return &models.User{
			ID:       user.Username,
			Username: user.Username,
